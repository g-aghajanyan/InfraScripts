...
Add-Content $report "<html>`n<head>`n<meta http-equiv='Content-Type' content='text/html; charset=iso-8859-1'>"
...
add-content $report  "}" 
add-content $report  "-->" 
add-content $report  "</style>`n</head>`n<body>"
...
add-content $report  "</td>`n</tr>`n</table>`n<table width='100%'>`n<tr bgcolor='IndianRed'>"
...
foreach ($DC in $DCServers) {
   ...
   if ( Test-Connection -ComputerName $DC -Count 1 -ErrorAction SilentlyContinue ) {
      ...
      if($sysvol.state -like "Running") {
         ...
         Slack $DC "Replication Test TimeOut --- Test manually by running DCDiag /test:Replications /s:$DC"
         stop-job $sysvol
      } else {
         $sysvol1 = Receive-job $sysvol
...
# Close HTMl 
Add-Content $report "</tr>`n</table>`n</body>`n</html>"

# Send Email
$subject = "Active Directory Health Monitor" 
$body = Get-Content "C:\temp\ADreport.htm" 
Send-MailMessage -From $from -To $to -Subject $subject -Body "$body" -BodyAsHtml -SmtpServer $smtpsrv
